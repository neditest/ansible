...
def get_host_groups_from_cloud(inventory):
    ...
    for server in inventory.list_hosts():
        ...
        if 'managed' not in metadata or metadata['managed'] != 'yes':
            continue
    ...
    for server_id in hostvars.keys():
        hostvars[server_id]['ansible_ssh_hosts'] \
            = _get_ansible_ssh_hosts_from_hostvars(hostvars)
    groups['_meta'] = {'hostvars': hostvars}
    return groups

def _get_ansible_ssh_hosts_from_hostvars(hostvars):
    ansible_ssh_hosts = collections.defaultdict(list)
    for server_id in hostvars.keys():
        #ansible_ssh_hosts[hostvars[server_id]['ansible_ssh_host']] \
        #    = hostvars[server_id]['openstack']['human_id']
        hostname = hostvars[server_id]['openstack']['human_id']
        ansible_ssh_hosts[hostname].append(
            hostvars[server_id]['ansible_ssh_host'])
    return ansible_ssh_hosts
